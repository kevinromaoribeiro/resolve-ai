...
# O UNICO conjunto de comandos que atravessa o gate antes do aceite.
# Todos protegem o usuario ou explicam o tratamento — nenhum grava dado novo
# nem vende nada. "assinar" e "meu nome e X" ficam DE FORA de proposito.
_CMD_PRE_ACEITE = {"apagar meus dados", "apagar dados", "excluir meus dados",
                   "deletar meus dados", "privacidade", "termos", "lgpd",
                   "meus dados", "ajuda"}

# Comandos que NUNCA podem ser guardados na fila pre-aceite e replayados
# depois. Todos abrem confirmacao destrutiva ou mexem em assinatura: replay
# arma o CONFIRM (que nao tem TTL) por conta de uma frase antiga, e um "sim"
# ou "apagar" solto dias depois executa a acao. Comando e ordem do momento.
...
# Mensagens que a pessoa mandou ANTES de aceitar. Em memoria de proposito:
# e efemera e nao vira base de dado sem consentimento — gravar no SQLite
# seria justamente o que o aceite ainda nao autorizou.
# E uma LISTA por telefone: a primeira versao sobrescrevia, entao quem
# mandava 3 demandas recebia a promessa "guardei" 3 vezes e so a ultima
# sobrevivia.
PRE_ACEITE: dict = {}
PRE_ACEITE_MAX = 500        # teto de telefones (anti-vazamento de memoria)
PRE_ACEITE_MAX_MSGS = 5     # teto de mensagens guardadas por pessoa
PRE_ACEITE_TTL_S = 3600     # some depois de 1h sem aceitar

# Telefones que devem receber o pedido de reconsentimento ANEXADO na proxima
# resposta. Mesmo padrao do TRIAL_VENCIDO.
RECONSENTIR: dict = {}

# Telefones que acabaram de recusar e tiveram os dados apagados. Sem isto,
# recusar e mandar "oi" recria o usuario e dispara o onboarding INTEIRO de
# novo — 6 mensagens do bot para 6 do usuario, o ritmo de rajada que rendeu
# 2 restricoes da Meta em 04/08.
RECEM_APAGADOS: dict = {}
CARENCIA_POS_RECUSA_S = 300     # 5 min sem reabrir o onboarding completo

# Ultimo envio da abertura completa, por telefone. Sem isto cada "oi" de quem
# esta parado no aceite dispara DUAS mensagens.
ULTIMO_AVISO_LGPD: dict = {}
REABRIR_ONBOARDING_S = 600  # 10 min ate repetir a abertura inteira

# Confirmacao pendente que foi cancelada porque a pessoa mandou outra coisa.
CANCELADO_AVISO: dict = {}

# Telefones cuja fila pre-aceite esta sendo reprocessada AGORA. Distingue
# "mensagem que a pessoa acabou de mandar" de "mensagem antiga sendo colhida".
EM_REPROCESSO: set = set()
_SEM_CONFIRM = object()     # sentinela: distingue "nao tinha" de "tinha None"

# phone -> (user_id, n_itens_antes, [demandas]). Conferido no webhook depois
# que a resposta ja saiu: as demandas guardadas viraram item MESMO?
CONFERIR_FILA: dict = {}
...
def _get_or_create_user(phone: str, push_name: str = "") -> tuple[dict, bool]:
    ...
    # por este caminho SEM ser `is_new` — resultado: o dono resetava, mandava
    # "oi", e a mensagem ia direto pro LLM sem nenhum aceite. Dois estragos:
    # um caminho processando dado sem consentimento, e o dono testando a M1.2
    # sem nunca ver a M1.2.
    # Todo usuario nasce parado no aceite. Sem excecao, nem pro dono.
...
def _handle_commands(user: dict, phone: str, text: str) -> Optional[str]:
    ...
    if phone in CONFIRM:
        ...
        # era criado — perda silenciosa. A confirmacao foi cancelada (o pop
        # ja aconteceu); a mensagem segue o fluxo e vira item, com um aviso
        # curto colado na resposta.
...
_COMANDOS_DO_BOT = {"feito", "feita", "pronto", "pronta", "pago", "paga",
                    ...
                    # M1.2 — as palavras dos botoes. Sem isto, o segundo
                    # toque em "Concordo" (o botao continua clicavel, e o
                    # msg_id e outro, entao o dedup nao pega) chega no passo
                    # "nome" e a pessoa passa a se chamar "Concordo".
...
def _handle_onboarding(user: dict, text: str) -> Optional[str]:
    ...
    if step in LGPD_STEPS:
        ...
        # handle_incoming — la existe acesso ao `content`, que e o que
        # permite REPROCESSAR a mensagem guardada antes do aceite. Aqui so
        # blindamos: se algum caminho novo chamar isto nesse estado, nao vaza.
...
def _resposta_de_botao(user: dict, phone: str, text: str) -> Optional[str]:
    ...
    so transmite o TITULO. O titulo volta como texto livre, ninguem trata,
    ...
    if _ACK_MUDAR_RE.match(t):
        ...
        if desc:
            return (f"Beleza — o que muda em *{desc}*?\\n"
                    f"Pode mandar s\u00f3 o que est\u00e1 errado "
                    f"(_\\"na verdade \u00e9 dia 20\\"_).")
...
def _nao_e_nome_de_formulario(nome: str) -> bool:
    ...
    if len(t) < 2 or len(t) > 60:
        return True
    if re.fullmatch(r"[\\d\\s./-]+", t):          # "30", "11/08"
        return True
    if _LOOKS_LIKE_QUESTION.search(t):
        return True
...
def _abrir_onboarding_lgpd(phone: str, texto_abertura: str,
    ...
    Existe como funcao unica porque ha TRES portas de entrada (usuario novo,
    payload da landing e saudacao de quem esta parado) e a auditoria pegou
    justamente uma delas escapando. Porta duplicada e regra que diverge.
    ...
    O painel mostrava o "oi" recebido e nenhuma resposta, exatamente na tela
    usada pra diagnosticar os 11 usuarios.
    """
    aviso = jornada.LGPD_AVISO.format(termos=TERMS_URL)
    # Marca aqui, e nao em quem chama: sao tres portas de entrada, e marcar
    # em uma so deixava a rajada viva pelas outras.
    ...
    for ok, txt in ((ok1, texto_abertura), (ok2, aviso)):
        try:
            ...
            import logging
            logging.getLogger("resolveai").warning(
                "[onboarding] falha ao logar mensagem no painel",
                exc_info=True)
    if not (ok1 and ok2):
...
def _enviar_avulsa(phone: str, texto: str, user_id=None) -> bool:
    """Manda uma mensagem fora do retorno do webhook, com log e alerta."""
...
def _conferir_fila_virou_item(phone: str) -> None:
    ...
    A fila volta pro motor como um bloco multi-linha, e quantos itens nascem
    dali e decisao do LLM — justamente onde o bot fez a promessa por escrito
    ("registro assim que voce confirmar"). Cinco linhas virando um item e
    perda silenciosa de quatro demandas.

    Nao conserta o motor: detecta a falha e faz barulho, em vez de deixar a
    pessoa descobrir no vencimento.
    """
    ...
    log = "; ".join(d[:40] for d in demandas)
    _alertar_dono(
        f"FILA PRE-ACEITE: {len(demandas)} demandas viraram {criados} "
        f"item(ns) — {faltam} pode(m) ter se perdido no motor. [{log}]",
        phone, log[:120])


def _purgar_pre_aceite() -> None:
    """Tira da memoria o que ficou para tras (quem abandonou o cadastro)."""
    agora = tempo.agora()
    for tel in list(PRE_ACEITE):
        fila = [(t, q) for t, q in PRE_ACEITE.get(tel) or []
...
def _pop_pre_aceite(phone: str) -> list:
    """Tira a fila de demandas pre-aceite e devolve LISTA de textos.
    ...
    passou a devolver a lista crua pro fluxo de texto e derrubava o webhook
    com AttributeError — 500, dedup engolindo o reenvio, e a demanda que o
    bot tinha ACABADO de prometer registrar perdida.
    """
    return [t for t, _q in (PRE_ACEITE.pop(phone, None) or [])]
...
def _colher_pre_aceite(user: dict, phone: str, fechou_cadastro: bool) -> list:
    ...
    A pessoa mandou "luz 187 vence dia 20" antes de aceitar os Termos. O bot
    respondeu, por escrito, "Guardei aqui e registro assim que voce
    confirmar". No fluxo organico ainda faltavam dois passos (nome e
    interesses), entao a promessa so pode ser cumprida AQUI.
    """
...
def _resolver_aceite(user: dict, phone: str, kind: str,
    ...
    if aceite is None:
        ...
        # "cancelar" como se fosse demanda e replayar depois do aceite
        # ARMAVA a confirmacao destrutiva: dias depois, um "apagar" solto
        # apagava a conta por causa de uma frase mandada antes de existir
        # cadastro. E CONFIRM nao tem TTL.
...
def _classify_message(msg: dict) -> tuple[str, str]:
    ...
    # RESPOSTA DE BOTAO.
    # O canal oficial (meta_cloud.to_evolution_shape) normaliza o clique para
    ...
    # nunca sai. Botao e o caminho principal do produto; ele nao pode depender
    # de um unico tradutor estar correto.
...
def handle_incoming(payload: dict) -> Optional[dict]:
    ...
    # Purga dos dicionarios efemeros, no topo do fluxo e nao escondida num
    # ramo: quando morava so dentro do gate, os mapas so eram varridos se
    # ALGUEM mandasse conteudo antes de aceitar.
    _purgar_pre_aceite()

    # OBS: o download da midia foi movido para DEPOIS do gate de aceite
    # (bloco 0a). Baixar significa decriptar o audio/foto da pessoa, e "sem
    ...
    if is_new:
        ...
        # rajada de mensagens e reinicia o trial a cada volta.
    ...
    # POSICAO IMPORTA. Este gate ja esteve ACIMA dos dois blocos anteriores,
    ...
    # Deterministico, e antes do motor. Sem isto o titulo do botao vira texto
    # livre e o LLM responde "Como vai?" a um "Isso mesmo" — foi o que
    # aconteceu com o Kevin em 11/08, duas vezes no mesmo dia.
    #
    # POSICAO IMPORTA. Este bloco ja esteve acima dos gates de acesso, e ali
    # ele: respondia a usuario BLOQUEADO (que deve levar silencio), roubava a
    # mensagem de reativacao de quem CANCELOU, e engolia a resposta de uma
    # decisao PENDENTE, deixando o PENDING travado para sempre.
...
try:
    ...
    async def webhook(request: Request):
        ...
        try:
            ...
            _alertar_dono(f"ERRO NAO TRATADO no webhook: {e!r}", num, content)
        ...
        # a resposta. O item da pessoa ja foi criado no fluxo normal.
        # O pop acontece SEMPRE que ha marcador, mesmo se reply for None —
        # senao o aviso fica preso e aparece dias depois, colado numa
        ...
        # resposta. A pessoa e atendida primeiro; o pedido de aceite vem
        # depois. O log_dispatch e gravado AQUI, no momento em que o texto
        # realmente entra na resposta — gravar na marcacao queimava o pedido
        # do dia mesmo quando a resposta era None (usuario bloqueado).
